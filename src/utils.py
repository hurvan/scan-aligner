...
def interpolate_to_common_timestamps2(devices, detectors):
    ...
    for det, (det_ts, det_data) in detectors.items():
        if len(det_ts) == 0:
            continue
        assert len(det_ts) == len(det_data), "Data and timestamps must be the same length"
        ...
        for dev, (dev_ts, dev_data) in devices.items():
            if len(dev_ts) == 0:
                continue
            assert len(dev_ts) == len(dev_data), "Data and timestamps must be the same length"
            ...
            if len(dev_ts) == 1:
                ...
                interp = interp1d(dev_ts, dev_data, kind='linear', fill_value=fill_value, bounds_error=False)
...
if __name__ == "__main__":
    ...
    common_ts, interp_funcs, interp_data = interpolate_to_common_timestamps(dev_ts, dev_data, det_ts, det_data)
